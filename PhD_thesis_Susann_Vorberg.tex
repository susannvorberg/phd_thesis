...
\section{Approximation of the Gradient with Contrastive
...
\item
  ...
  is at optimum -Just performing a single Gibbs sampling step away from
  the original data vectors r n moves the sampled r n,t into the
  direction of the equilibrium posterior distribution, and therefore
  promises to yield a similar direction of change as the true gradient
  of the likelihood.
\item
...
decreasing the step size, which is also called learning rate, of the
parameter updates at every iteration, stochastic gradient descent most
likely will converge to the global minimum for convex objective
functions
{[}\protect\hyperlink{ref-Ruder2017}{119},\protect\hyperlink{ref-Bottou2012}{120}{]}.
...
manual tuning {[}\protect\hyperlink{ref-Schaul2013}{121}{]}. If the step
size is chosen too small, progress will be unnecessarily slow, if it is
chosen too large, the optimum will be overshot and parameter estimates
might diverge. Another key challenge is that one specific step size
often is not an optimal choice for all parameters. Because the
magnitudes of gradients might vary considerably for different
parameters, e.g.~because of sparse data, different parameters require
different optimal step sizes. Another disadvantage of
\protect\hyperlink{abbrev}{SGD} methods is that it is not easy to
identify suitable convergence criteria.
...
There exist many variants of stochastic gradient descent algorithms that
deal with the aforementioned challenges e.g.~speeding up convergence
rates using momentum or defining adaptive learning rates for each
parameter {[}\protect\hyperlink{ref-Ruder2017}{119}{]}. One of these
\protect\hyperlink{abbrev}{SGD} variants is Adaptive Moment Estimation
(\emph{ADAM}) {[}\protect\hyperlink{ref-Kingma2014}{122}{]}, an
algorithm that computes adaptive learning rates for each parameter
including momentum (see methods section
\ref{methods-full-likelihood-adam} for details). A major advantage is
that \emph{ADAM} does not require tuning many hyperparameters as the
default values have been found to work quite well.

\subsection{Optimizing with ADAM}\label{full-likelihood-adam}

\begin{itemize}
\item
  explain adam with its advantage of adaptive learning rates
\item
  define optimal learning rate and decay dependent on neff
  --\textgreater{} see methods
\item
  violates sum\_wijab condition, sum\_wijab could be used as convergence
  criterium but practice shows that it is esp hard to reach for huge
  Neff, L? use change in parameter values as criteria instead
\item
...
\subsection{Optimizing with vanilla stochastic gradient
descent}\label{full-likelihood-sgd}

\begin{itemize}
\item
  need to manually tune learning rate and decay
\item
  no adaptivity --\textgreater{} harder to choose learning rate and
  decay than for adam
\item
  however sum\_wijab condition is not violated
\item
...
\subsection{Altering Gibbs sampling
Schemes}\label{altering-gibbs-sampling-schemes}
...
{[}\protect\hyperlink{ref-Jones2015a}{67},\protect\hyperlink{ref-He2017}{70},\protect\hyperlink{ref-Ma2015a}{123}{]}.
...
respectively {[}\protect\hyperlink{ref-Menze2009}{127}{]}.
...
{[}\protect\hyperlink{ref-Strobl2007}{128}{]}.
...
{[}\protect\hyperlink{ref-Miyazawa1999a}{129}{]}, rsa\_i amd rsa\_j =
solvent accessibility prediction for residues i and j with NetsurfP
{[}\protect\hyperlink{ref-Petersen2009a}{130}{]}, LiFang = mean pairwise
...
{[}\protect\hyperlink{ref-Sillitoe2015}{131}{]} database for
...
close to zero {[}\protect\hyperlink{ref-Carreira-Perpinan2005}{132}{]}.
...
400 gradients \(\wijab\) for \(a,b \in {1, ..., 20\}\) are not
...
{[}\protect\hyperlink{ref-Le2011}{133}{]}.
...
\emph{ADAM} {[}\protect\hyperlink{ref-Kingma2014}{122}{]} stores an
...
of the gradient, respectively. Because \(m_t\) and \(v_t\) are
initialized as vectors of zeros, the following bias correction terms are
supposed to counteract the initialization bias towards zero,
...
{[}\protect\hyperlink{ref-Chollet2015}{134}{]} and
\href{https://github.com/Lasagne/Lasagne/blob/master/lasagne/updates.py\#L565-L629}{Lasagne}
{[}\protect\hyperlink{ref-Dieleman2015}{135}{]} packages allow the use
of a linear annealing schedule for the learning rate \(\alpha\) of the
form \(\alpha = \frac{\alpha_0}{1 + \gamma t}\), for an initial learning
rate \(\alpha_0\), decay rate \(\gamma\) and timestep \(t\).
...
{[}\protect\hyperlink{ref-Bottou2012}{120}{]}.
...
with a small alignment (left plot), a small learning rate
\(1\mathrm{e}{-4}\) lets the optimization run very slowly and not reach
convergence within 5000 iterations. A large learning rate of
...
In contrast, for a protein with a big alignment (right plot) the effects
of the learning rate choice are much more pronounced. With a small
initial learning rate of \(1\mathrm{e}{-4}\) the optimization runs
slowly but almost converges within 5000 iterations. Using a big initial
learning rate of \(5\mathrm{e}{-3}\), the parameter estimates diverged
too far from the optimum and never reach the optimum. With learning
rates \(5\mathrm{e}{-4}\) and \(1\mathrm{e}{-3}\), the optimum is well
overshot at the beginning of the optimization but the parameter
estimates eventually converge as the learning rates decreases over time.
...
sequences is largest.

Following the observations I defined the initial learning rate
\(\alpha_0\) as a function of \protect\hyperlink{abbrev}{Neff}.\\
...
\(\alpha_0 \approx 3.5\mathrm{e}{-4}\). Mean precision (see XXXcolor in
in Figure \ref{fig:performance-cd-alphaopt}) is comparable to the
optimizations using \(\alpha_0 = 5e-4\) and \(\alpha_0 = 1e-3\). All
following analyses are conducted using the initial learning rate defined
as a funciton of \protect\hyperlink{abbrev}{Neff} as defined above.
...
rate schedules and decay rates using an initial learning rate defined as
a funciton of \protect\hyperlink{abbrev}{Neff} as in eq.
\eqref{eq:learning-rate-wrt-neff}.
...
\item
  ...
  \(\gamma \in \{1\mathrm{e}{-5}, 1\mathrm{e}{-4}, 1\mathrm{e}{-3}\}\)
\end{itemize}
...
{[}\protect\hyperlink{ref-Bengio2009}{136},\protect\hyperlink{ref-Tieleman2008}{137}{]}.
...
\protect\hyperlink{abbrev}{\emph{PCD}}{[}\protect\hyperlink{ref-Tieleman2008}{137}{]}
...
(v4.0){[}\protect\hyperlink{ref-Jones1999}{138}{]}\strut
\end{minipage} & \begin{minipage}[t]{0.18\columnwidth}\centering\strut
3\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.23\columnwidth}\raggedleft\strut
...
(v1.0){[}\protect\hyperlink{ref-Petersen2009a}{130}{]}\strut
\end{minipage} & \begin{minipage}[t]{0.18\columnwidth}\centering\strut
3\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.23\columnwidth}\raggedleft\strut
...
{[}\protect\hyperlink{ref-Robinson1991}{139}{]}\strut
\end{minipage} & \begin{minipage}[t]{0.18\columnwidth}\centering\strut
2\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.23\columnwidth}\raggedleft\strut
...
frequencies {[}\protect\hyperlink{ref-Robinson1991}{139}{]}\strut
\end{minipage} & \begin{minipage}[t]{0.18\columnwidth}\centering\strut
40\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.23\columnwidth}\raggedleft\strut
...
{[}\protect\hyperlink{ref-Jones1999}{138}{]}\strut
\end{minipage} & \begin{minipage}[t]{0.18\columnwidth}\centering\strut
6\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.23\columnwidth}\raggedleft\strut
...
{[}\protect\hyperlink{ref-Petersen2009a}{130}{]}\strut
\end{minipage} & \begin{minipage}[t]{0.18\columnwidth}\centering\strut
6\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.23\columnwidth}\raggedleft\strut
...
\end{minipage} & \begin{minipage}[t]{0.50\columnwidth}\raggedright\strut
Atchley Factors 1-5 {[}\protect\hyperlink{ref-Atchley2005}{140}{]}\strut
\end{minipage} & \begin{minipage}[t]{0.18\columnwidth}\centering\strut
10\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.23\columnwidth}\raggedleft\strut
...
Database} {[}\protect\hyperlink{ref-Kawashima2008}{141}{]}.\strut
\end{minipage} & \begin{minipage}[t]{0.18\columnwidth}\centering\strut
2\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.23\columnwidth}\raggedleft\strut
...
Chimera} {[}\protect\hyperlink{ref-Wimley1996}{142}{]}.\strut
\end{minipage} & \begin{minipage}[t]{0.18\columnwidth}\centering\strut
2\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.23\columnwidth}\raggedleft\strut
...
{[}\protect\hyperlink{ref-Cornette1987}{143}{]}.\strut
\end{minipage} & \begin{minipage}[t]{0.18\columnwidth}\centering\strut
2\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.23\columnwidth}\raggedleft\strut
...
Zhu\&Braun {[}\protect\hyperlink{ref-Zhu1999}{144}{]}\strut
\end{minipage} & \begin{minipage}[t]{0.18\columnwidth}\centering\strut
1\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.23\columnwidth}\raggedleft\strut
...
{[}\protect\hyperlink{ref-Fodor2004a}{145}{]} with and without APC\strut
\end{minipage} & \begin{minipage}[t]{0.18\columnwidth}\centering\strut
2\strut
\end{minipage}\tabularnewline
\bottomrule
...
{[}\protect\hyperlink{ref-Bernard2009}{146}{]}. Stochasticity introduced
...
{[}\protect\hyperlink{ref-Louppe2014}{147}{]}
...
{[}\protect\hyperlink{ref-Pedregosa2011}{148}{]} and trained the models
...
Randomly selecting 15\% of features (=38 features) and requiring at
...
{[}\protect\hyperlink{ref-Menze2009}{127}{]}. For example, during the
...
119. Ruder, S. (2017). An overview of gradient descent optimization
algorithms. arXiv. Available at: \url{http://arxiv.org/abs/1609.04747}.

\hypertarget{ref-Bottou2012}{}
120. Bottou, L. (2012). Stochastic Gradient Descent Tricks. In Neural
...
124. Ho, T.K. (1998). The random subspace method for constructing
...
128. Strobl, C., Boulesteix, A.-L., Zeileis, A., and Hothorn, T. (2007).
...
129. Miyazawa, S., and Jernigan, R.L. (1999). Self-consistent estimation
...
130. Petersen, B., Petersen, T.N., Andersen, P., Nielsen, M., and
...
131. Sillitoe, I., Lewis, T.E., Cuff, A., Das, S., Ashford, P., Dawson,
...
132. Carreira-Perpiñán, M. a, and Hinton, G.E. (2005). On Contrastive
...
133. Le, Q.V., Ngiam, J., Coates, A., Lahiri, A., Prochnow, B., and Ng,
...
134. Chollet, F. and others (2015). Keras. Available at:
\url{https://github.com/fchollet/keras}.

\hypertarget{ref-Dieleman2015}{}
135. Dieleman, S., Schlüter, J., Raffel, C., Olson, E., Sønderby, S.K.,
...
136. Bengio, Y., and Delalleau, O. (2009). Justifying and Generalizing
...
137. Tieleman, T. (2008). Training Restricted Boltzmann Machines using
Approximations to the Likelihood Gradient. Proc. 25th Int. Conf. Mach.
Learn. \emph{307}, 7.

\hypertarget{ref-Jones1999}{}
138. Jones, D.T. (1999). Protein secondary structure prediction based on
...
139. Robinson, A.B., and Robinson, L.R. (1991). Distribution of
...
140. Atchley, W.R., Zhao, J., Fernandes, A.D., and Drüke, T. (2005).
...
141. Kawashima, S., Pokarowski, P., Pokarowska, M., Kolinski, A.,
...
142. Wimley, W.C., and White, S.H. (1996). Experimentally determined
...
143. Cornette, J.L., Cease, K.B., Margalit, H., Spouge, J.L., Berzofsky,
...
144. Zhu, H., and Braun, W. (1999). Sequence specificity, statistical
...
145. Fodor, A.A., and Aldrich, R.W. (2004). Influence of conservation on
calculations of amino acid covariance in multiple sequence alignments.
Proteins \emph{56}, 211--21. Available at:
\url{http://www.ncbi.nlm.nih.gov/pubmed/15211506}.

\hypertarget{ref-Bernard2009}{}
146. Bernard, S., Heutte, L., and Adam, S. (2009). Influence of
...
147. Louppe, G. (2014). Understanding Random Forests: From Theory to
Practice. Available at: \url{http://arxiv.org/abs/1407.7502}.

\hypertarget{ref-Pedregosa2011}{}
148. Pedregosa, F., Varoquaux, G., Gramfort, A., Michel, V., Thirion,
