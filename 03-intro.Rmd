...
Given a protein with 100 residues and assuming 3 degrees of freedom for each bond, there woud be 10^27 configurations that could not be sampled in a lifetime, even when assuming ridicuously high rates.
Yet, proteins fold within miliseconds. 
...
Nuclear magnetic resonance (NMR) studies the protein in a physiological environment in solution and enables the study of intramolecular dynamics,reaction kinetics or protein folding as ensembles of protein structures can be observed. 
There is an upper size limit of about 25 kDa for efficient use of NMR spectroscopy[@Billeter2008]. 
Recently, Cryo-EM has undergone a "resolution revolution" and macromolecules have been solved to near-atomic resolutions [@Egelman2016]. 
...
This trend illustrates the need of computational approaches that can complement experimental structural biology efforts in order to close this gap.
Over the last decades template based approaches have matured to a point where they are able to generate high-resolution structural models that are routinley and conveniently used in life-science research and by the biological community [@Schwede2013; @BKC2016].
...
Albeit purely *de novo* approaches are hampered by the combinatorial explosion of possible conformations for larger proteins, with the availability of sparse low-resolution experimental constraints, from NMR and cross-linking, the search space can be confined. 
Integrative approaches combining structural information from heterogenous sources have proven to be a powerful approach.


In the last years, very promising results have been obtained by using predicted contacts as distance constraints.
It has long been known that native contacts can be used to reliably reconstruct the protein fold.
A protein contact map is a 2D representation of the protein fold. 
...
```{r contact-map, echo = FALSE, out.width='90%', fig.align = 'center', fig.cap = '(ref:caption-contact map)'}
knitr::include_graphics("img/intro/contact_map_1aho.png")
```
