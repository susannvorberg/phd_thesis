...
Ever since, it is regarded as the biggest challenge in structural bioinformatics to realiably predict a protein's structure given only its amino acid sequence [@Samish2015]. 
...
Consequently, *de novo* protein structure prediction is typically limited to small proteins.
...
About 9% of protein structures have been solved using [NMR](#abbrev) and less than 1% using [EM](#abbrev) (see \@ref(fig:seq-str-gap)).
...
At the beginning of 2017, Illumina announced the launch of their latest high-throughput sequencing technology, NovaSeq, which is capable of sequencing ~48 human genomes in parallel at 30x coverage within ~45hours [@NovaSeqSystemSpecifications].
...
The UniProtKB, the leading resource for protein sequences, currently holds more than 90 million sequence entries (October 2017) [@TheUniProtConsortium2013].

The resultant gap between the number of protein structures and protein sequences is constantly widening despite tremendous efforts in automating experimental structure determination and new developments such as electron crystallography (see Figure \@ref(fig:seq-str-gap) [@Schwede2013; @Clabbers2017].

```{r seq-str-gap, echo = FALSE, screenshot.alt="img/intro/pdb_uniprot_stats.png",out.width = '100%', screenshot.alt="img/amino_acid_physico_chemical_properties_venn_diagramm.png", fig.cap = 'Yearly growth of number of solved structures in the PDB[@Berman2000] and protein sequences in the Uniprot[@TheUniProtConsortium2013].'}
knitr::include_url("img/intro/pdb_uniprot_stats.html", height = "750px")
```


The aformentioned trends illustrate the need of computational methods to complement experimental structural biology efforts to characterize the protein universe [@Schwede2013].
Homology modelling is an established technology when homologue protein srtucture can be detected and *de novo* structure prediction aiming at predicting protein structures from sequence alone has gained new speed with the co-evolution methods being able to reliably predict accurate contacts [@Schwede2013].
...
given a protein with 100 residues and assuming 3 degreed of freedom for each bond, there woud be 10^27 configurations that could not be sampled in a lifetime, even when assuming ridicuously high rates.
The paradox, known as Levinthals paradox is that proteins fold in miliseconds 
