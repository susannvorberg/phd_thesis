...
- theoretical upper boundary on precision: simply not all contacts within 8A interact with each other and therefore will never be detected using covariance methods --> influences sensitivity.
- strongly conserved residues can also not show covariance signals --> influences sensitivity
...
Maximizing the entropy $S= -\sum_{\seq} p(\seq) \log p(\seq)$ of the distribution subject to the constraints given in eq. \@ref(eq:emp-freq) via Lagrange multipliers results in the formulation of an exponential model known as *Potts model* or its generalized form a *Markov Random Field*:
