...
(ref:caption-stats-pll-5comp-100k) Statistics for the hyperparameters, $\gamma_k(\cij)$, $\muk$ and $\Lk$ obtained after 2552 iterations. Trained on 100,000 residue pairs per contact class for a three component Gaussian mixture and using *pseudo-likelihood* couplings to estimate the Hessian. **Left** Component weights $\gamma_k(\cij)$ for residue pairs not in physical contact ($\cij \eq 0$) and true contacts ($\cij \eq 1$). **Center** Distribution of the 400 elements in the mean vectors $\muk$. **Right** Distribution of the 400 standard deviations corresponding to the square root of the diagonal of $\Lk^{-1}$.

```{r stats-pll-5comp-100k, echo = FALSE, out.width = '100%', fig.cap = '(ref:caption-stats-pll-5comp-100k)'}
knitr::include_graphics("img/bayesian_model/pll/5/stats-pll-5comp-100k_contactthr25.png")
```
