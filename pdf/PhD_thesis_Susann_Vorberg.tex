...
Due to the complexity of the normalization constant it is infeasible to
infer the model parameters via maximum-likelihood estimation. The most
popular \protect\hyperlink{abbrev}{DCA} approaches optimize the
pseudo-likelihood instead but it is unknown how well it approximates the
full likelihood in case protein families have only few members. In this
chapter I tested an alternative approach to infer the \emph{Potts} model
parameters called \emph{contrastive divergence}. {[}CD{]} optimizes the
full likelihood of the \emph{Potts} model by approximating the gradient
of the the full likelihood with short Gibbs chains. However, it turned
oit that pLL performs as well as CD, also in low data scanrios, given he
fac thtat the improvement of CD can be attributed to couplings between
highly conserved pairs.
...
computed as the \protect\hyperlink{abbrev}{APC} corrected Frobenius norm
of the couplings, and the ovaral ranking of residue pairs. This was
shwon in the last sectionL: the ranking of pseudo-likelihood and CD
scores is very similar. - strongest signals are sufficient
{[}{\textbf{???}}{]} - finetuning lower signals not relevant

Sophisitcated methods are required to use this fientuned information?
--\textgreater{} bayes? Furthermore, it can be speculated that the
heuristic contact score that has empirically been found to work very
well for pseudo-likelihood couplings, might not be an appropriate choice
for couplings computed with \protect\hyperlink{abbrev}{CD}.
...
fixed at \(\mu_0=0\) on datasets of different sizes (see method section
\ref{dataset-training-bayesian-model} for details). During optimization
the value of the negative log likelihood is monitored not only on the
training but also on a validation set to ensure the model is not
overfitting the training data (see Appendix Figure
\ref{fig:neglogll-pll-3comp}). Eventhough, optimization according to a
stringent convergence criterion (see method section
\ref{dataset-training-bayesian-model}) has only been observed for
training Gaussian mixtures with \(K \eq 3\) components and only using
the smaller datasets (10,000 and 1000,000 residue pairs per contact
class), the parameters for all optimization runs did not change
substantially after several hundred iterations and the resultant
Gaussian mixtures are consistent regardless of the dataset size and
independent repeated trainings. In the following, analysis of the
optimization results is conducted for optimization on the dataset with
100,000 residue pairs per contact class and using pseudo-likelihood
couplings for estimation of the Hessian.

Figure \ref{fig:stats-pll-3comp-100k} shows the statistics of the
hyperparameters for a three component Gaussian mixture. The zeroth
component, with \(\mu_0=0\) has a weight of 0.88 for the non-contact
class, whereas it has only weight 0.5 for the contact class. This is
expected given that the couplings \(\wijab\) for non-contacts have a
much tighter distribution around zero than contacts. Component 2 has on
average the highest standard deviations and for several dimensions this
component is located far off from zero, e.g.~dimension EE has
...
(\(g_2(1) \eq 0.14\)). Statistics of the hyperparameters of the Gaussian
mixture obtained after learning for several hundred of iterations on the
other datasets is shown in Appendix Figures
\ref{fig:stats-pll-3comp-10k}, \ref{fig:stats-pll-3comp-300k},
\ref{fig:stats-pll-3comp-500k}. Furthermore, the statistics of
hyperparameters for the Gaussian mixture model based on couplings
optimized with contrastive divergence are very consistent and are shown
in Appendix Figures \ref{fig:stats-cd-3comp-100k} ans
\ref{fig:stats-cd-3comp-10k}.
...
\ref{fig:vis1d-aa-pll-3comp-300k} and \ref{fig:vis1d-aa-pll-3comp-500k})
Furthermore, the distributions for the model based on contrastive
divergence couplings are also very similar and is shown in Appendix
Figure \ref{fig:vis1d-cd-3comp-100k}

The two-dimensional projections of the Gaussian mixture are shown in
Figure \ref{fig:vis2d-pll-3comp-100k} for several pairs of couplings.
...
of 0.14 for contacts and is mainly responsible for the positive coupling
...
Figure \ref{fig:vis2d-pll-3comp-100k} show the distribution of amino
...
non-contacts. The two-dimensional distributions for the model based on
contrastive divergence couplings is also very similar and is shown in
Appendix Figure \ref{fig:vis2d-cd-3comp-100k}
...
weight \(g_k(0)\)).}\label{fig:vis2d-pll-3comp-100k}
\end{figure}
...
elements in the mean vectors \(\muk\). \textbf{Right} Distribution of
the 400 standard deviations corresponding to the square root of the
diagonal of \(\Lk^{-1}\).}\label{fig:stats-pll-3comp-500k}
\end{figure}
...
mixture and using contrastive divergence couplings to estimate the
Hessian. \textbf{Left} Component weights \(\gamma_k(\cij)\) for residue
pairs not in physical contact (\(\cij \eq 0\)) and true contacts
(\(\cij \eq 1\)). \textbf{Center} Distribution of the 400 elements in
the mean vectors \(\muk\). \textbf{Right} Distribution of the 400
...
projection for pair (V,I). \textbf{Top Right} One dimensional projection
for pair (F,W). \textbf{Bottom Left} One dimensional projection for pair
(E,R). \textbf{Bottom Right} One dimensional projection for pair (E,E).}\label{fig:vis1d-aa-pll-3comp-300k}
\end{figure}
...
projection for pair (V,I). \textbf{Top Right} One dimensional projection
for pair (F,W). \textbf{Bottom Left} One dimensional projection for pair
(E,R). \textbf{Bottom Right} One dimensional projection for pair (E,E).}\label{fig:vis1d-aa-pll-3comp-500k}
\end{figure}
