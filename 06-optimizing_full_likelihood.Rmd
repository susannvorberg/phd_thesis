...
Evolving the Gibbs chains for more than k=1 steps results in a slight drop in performance, just as it has been observed for [CD](#abbrev) (Figure \@ref(fig:precision-pcd) shows performance for k=10 steps).
...
In contrast to the findings in literature, neither [CD](#abbrev)-k with k>1 Gibbs steps nor [PCD](#abbrev) does improve performance with respect to precision of the top ranked contact predictions. 
...
The previous sections dealed intensively with the hyperparameter optimization for the stochastic gradient decent optimizer and hyperparameters of the [CD](#abbrev) model itself. 
...
[CD](#abbrev)-1 generates sequence samples according to the current state of the model by evolving Gibbs chains, that have been initialized at data samples, for one full step.
Eventhough, the *ADAM* optimizer did not improve perfomance, it is still likely that appropriate modifications of the optimization procedure might lead to improvments, e.g. averaging [@Ma2016]. 
...
It might be worth to assess convergence with more sophisticated metrics than the L2 norm of the coupling parameters, $||\w||_2$, liek the EB-criterion proposed by Mahsereci et al. [@Mahsereci2017].
...
It can be speculated that the heuristic contact score, computed as the [APC](#abbrev) corrected Frobenius norm of the couplings, that has empirically been found to work very well for pseudo-likelihood couplings, might not be an appropriate choice for couplings computed with [CD](#abbrev).
Furthermore, it is possible that the improved gradient, obtained by generating more samples, helps to finetune the parameters which has no effect on the contact score and the ovaral ranking of residue pairs. 
...
A final benchmark over a larger set of proteins (2000 proteins from subsets 9 and 10 from dataset described in method section \@ref(dataset)) reveals that contact predictions obtained by maximizing the pseudo-likelihood and by optimizing the full likelihood with contrastive divergence perform similar (see Figure \@ref(fig:precision-cd-final)). 
...
```{r contact-maps-1c75a00, echo = FALSE, out.width = '48%', fig.align='center', fig.show='hold', fig.cap = '(ref:caption-contact-maps-1c75a00)'}
...
The left plot in Figure \@ref(fig:ranking-pll-cd-1c75a00) compares the ordered scores of both methods that lie very close to the diagonal which indicates that both distribution are very similar. 
...
```{r precision-cd-final-neff, echo = FALSE, screenshot.alt="img/full_likelihood/final/meanprecision_by_neff.png", out.width = '90%', fig.align='center', fig.cap = 'better for small N'}
