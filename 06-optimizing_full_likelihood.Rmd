...
In this chapter I tested an alternative approach to infer the *Potts* model parameters called *contrastive divergence*.
[CD] optimizes the full likelihood of the *Potts* model by approximating the gradient of the the full likelihood with short Gibbs chains. 
However, it turned oit that pLL performs as well as CD, also in low data scanrios, given he fac thtat the improvement of CD can be attributed to couplings between highly conserved pairs.
...
This was shwon in the last sectionL: the ranking of pseudo-likelihood and CD scores is very similar. 
  - strongest signals are sufficient [@cocoo]
    - finetuning lower signals not relevant
  
Sophisitcated methods are required to use this fientuned information? --> bayes?
