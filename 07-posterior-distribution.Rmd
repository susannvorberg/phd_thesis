...
I trained models with $K \in \{3,5,10\}$ Gaussian mixture components with diagonal precision matrices $\Lk$ and a zero-component that is fixed at $\mu_0=0$ on datasets of different sizes (see method section \@ref(dataset-training-bayesian-model) for details).
During optimization the value of the negative log likelihood is monitored not only on the training but also on a validation set to ensure the model is not overfitting the training data (see Appendix Figure \@ref(fig:neglogll-pll-3comp)).
Eventhough, optimization according to a stringent convergence criterion (see method section \@ref(dataset-training-bayesian-model)) has only been observed for training Gaussian mixtures with $K \eq 3$ components and only using the smaller datasets (10,000 and 1000,000 residue pairs per contact class), the parameters for all optimization runs did not change substantially after several hundred iterations and the resultant Gaussian mixtures are consistent regardless of the dataset size and independent repeated trainings.
In the following, analysis of the optimization results is conducted for optimization on the dataset with 100,000 residue pairs per contact class and using pseudo-likelihood couplings for estimation of the Hessian. 

Figure \@ref(fig:stats-pll-3comp-100k) shows the statistics of the hyperparameters for a three component Gaussian mixture. 
...
Statistics of the hyperparameters of the Gaussian mixture obtained after learning for several hundred of iterations on the other datasets is shown in Appendix Figures \@ref(fig:stats-pll-3comp-10k), \@ref(fig:stats-pll-3comp-300k), \@ref(fig:stats-pll-3comp-500k).
Furthermore, the statistics of hyperparameters for the Gaussian mixture model based on couplings optimized with contrastive divergence are very consistent and are shown in Appendix Figures \@ref(fig:stats-cd-3comp-100k) ans \@ref(fig:stats-cd-3comp-10k).
...
Figure \@ref(fig:vis1d-pll-3comp-100k) shows several one-dimensional projections of the 400 dimensionl Gaussian mixture with three components.
...
The model learned on larger datasets produces very similar distributions (see Appendix Figures \@ref(fig:vis1d-aa-pll-3comp-300k) and \@ref(fig:vis1d-aa-pll-3comp-500k))
Furthermore, the distributions for the model based on contrastive divergence couplings are also very similar and is shown in Appendix Figure \@ref(fig:vis1d-cd-3comp-100k)

The two-dimensional projections of the Gaussian mixture are shown in Figure \@ref(fig:vis2d-pll-3comp-100k) for several pairs of couplings.
...
Component 2 has a weight of 0.14 for contacts and is mainly responsible for the positive coupling between E-R and R-E.
...
The plots at the bottom of Figure \@ref(fig:vis2d-pll-3comp-100k) show the distribution of amino acid pairs I-L and V-I according to the Gaussian mixture model for contacts (component weight $g_k(1)$) as well as for non-contacts (component weight $g_k(0)$). 
...
The two-dimensional distributions for the model based on contrastive divergence couplings is also very similar and is shown in Appendix Figure \@ref(fig:vis2d-cd-3comp-100k)
...
(ref:caption-vis2d-pll-3comp-100k) Visualisation of two-dimensional projections of the three-component Gaussian mixture model for the contact-dependent coupling prior. Hyperparameters, $\gamma_k(\cij)$, $\muk$ and $\Lk$, have been trained on 100,000 residue pairs per contact class. 10,000 values have been samples from the Gaussian mixture model. Light blue: values that have been generated by zero component. Dark blue: values that have been generated by Gaussian component 1. Light green: values that have been generated by Gaussian component 3. **Top Left** Two-dimensional projection for pairs (E,R) and (R-E) for contacts (using component weight $g_k(1)$). **Top Right** Two- dimensional projection for pairs (E,E) and (R,E) for contacts (using component weight $g_k(1)$).  **Bottom Left** Two-dimensional projection for pairs (I,L) and (V,I) for contacts (using component weight $g_k(1)$). **Bottom Right** Two-dimensional projection for pair (I,L) and (V,I) for non-contacts (using component weight $g_k(0)$).  
